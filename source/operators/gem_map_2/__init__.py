...
class VIEW3D_OT_gem_map_2(preferences.ReportLangEnum, Operator):
    bl_label = "Gem Map 2"
    bl_description = "Compose gem table and map it to gems in the scene using the new renderer"
    bl_idname = "view3d.jewelcraft_gem_map_2"
    ...
    def modal(self, context, event):
        if self.is_rendering:
            ...
            if event.type in {"ESC", "RET", "SPACE", "NUMPAD_ENTER"}:
                ...
                overlays.gem_map_2.handler_del()
                ...
                overlays.gem_map_2.handler_del()
                overlays.gem_map_2.handler_add(self, context, is_overlay=False, use_select=self.use_select, use_mat_color=self.use_mat_color)
                self.region.tag_redraw()
    ...
    def execute(self, context):
        ...
        from ..gem_map import onscreen, report_proc
        ...
        context.window_manager.jewelcraft.show_gem_map_2 = False
        overlays.gem_map_2.handler_add(self, context, is_overlay=False, use_select=self.use_select, use_mat_color=self.use_mat_color)
